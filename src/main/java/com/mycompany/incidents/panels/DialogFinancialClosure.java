...
public class DialogFinancialClosure extends javax.swing.JDialog implements Runnable {
    ...
    private void insertInFinalReportDTO(FinalReportDTO sourceDTO) {
        ...
        if(targetDTO==null){//no se encuentra
            ...
            targetDTO.setValorGW(sumarDouble(targetDTO.getValorGW(),sourceDTO.getValorGW()));            
            targetDTO.setReaseguroGW(sumarDouble(targetDTO.getReaseguroGW(),sourceDTO.getReaseguroGW()));
    ...
    private void insertGwInfoInDB(String rowInfo, ArrayList<Object> columnIdentifiers) throws Exception {
        ...
        aDTO.setValorGW(IncidentsUtil.determineDoubleValue(columnIdentifiers.get(6), rowInfoSplit));        
        aDTO.setReaseguroGW(IncidentsUtil.determineDoubleValue(columnIdentifiers.get(7), rowInfoSplit));        
        aDTO.setMoneda(IncidentsUtil.determineStringValue(columnIdentifiers.get(8), rowInfoSplit));
        
        insertInFinalReportDTO(aDTO);
        ...
        if (currentClosure != null && clave != null) {
            currentClosure.setValorCienGw(IncidentsUtil.determineDoubleValue(currentClosure.getValorCienGw())
                    + IncidentsUtil.determineDoubleValue(aDTO.getValorGW()));
            currentClosure.setValorReasGw(IncidentsUtil.determineDoubleValue(currentClosure.getValorReasGw())
                    + IncidentsUtil.determineDoubleValue(aDTO.getReaseguroGW()));
            ...
            newClosure.setValorCienGw(aDTO.getValorGW());
            newClosure.setValorReasGw(aDTO.getReaseguroGW());
